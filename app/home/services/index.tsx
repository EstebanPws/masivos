...
import { mockReferencePay } from "@/mocks/referenceService";
...
export default function Page() {
  ...
  const [labelReference, setLabelReference] = useState("Número de referencia de pago (sin guiones)")
      ...
      .then((response) => {
        ...
        if (data.data.length !== 0) {
          const listInvoices = data.data.map((invoice: any) => {
            const referencePay = mockReferencePay.filter((refPay) => refPay.CONVENIO.toLocaleLowerCase().includes(invoice.name.toLocaleLowerCase()));
            const labelRef = referencePay.length !== 0 ? referencePay[0]["REFERENCIA DE PAGO"] : "Número de referencia de pago (sin guiones)";
    ...
    await instanceWallet.post('product/query', body)
      .then((response) => {
        ...
        if (data.data.payload) {
          ...
          setMessageError("No se encontró la referencia de la factura.");
          setShowError(true);
          setTypeMessage('error');
          setTypeStepInvoice(0);
          setTypeFinish(0);
  ...
  const handleSetRefPay = (ref: string) => {
    setLabelReference(ref);
  }
  ...
  return (
    <ViewFadeIn isWidthFull>
      ...
      <View style={styles.container}>
        ...
        {(packageService === '82' && !isSearchInvoice) && (
          <View style={styles.row}>
            <View style={styles.viewSearch}>
              <Inputs
                label="ingresa el nombre de la empresa a la que desea realizar el pago de la factura."
                isSecureText={false}
                isRequired={true}
        ...
        <View style={[styles.services, packageService === '82' ? { maxHeight: isSearchInvoice ? '100%' : '58%' } : null]}>
              ...
              {(packageService === '82' && !loadSearch && isSearchInvoice) && (
                <View style={[isProductsFinalSelected ? styles.pH5 : null]}>
                  ...
                  <View style={styles.mb5Form}>
                    {typeStepInvoice === 0 && (
                      <Text style={[{ fontWeight: 700, textAlign: 'center' }, styles.mb5]}>¿Cúal es mi referencia de pago?</Text>
                    )}
                    <Inputs
                      label={typeStepInvoice === 0 ? labelReference : 'Referencia de pago'}
                  ...
                  <View style={styles.mb5}>
                    <ButtonsPrimary
                      label={typeStepInvoice === 0 ? "Siguiente" : "Pagar"}
